...
function composeSimple(mods: any[]) {
  ...
  return (Base: ComponentType<any>) => { 
    function SimpleComposeWrapper(props: Record<string, any>) {
      ...
      //===================
      //===================
      var nextCompactableClass: string = entity();
      //nextCompactableClass = nextCompactableClass(modifiers);
      for ( var inMod in modifiers){
        nextCompactableClass = nextCompactableClass
          + ' ' 
          //+ entity().split('__')[0]
          //+ '_'
          //+ inMod
